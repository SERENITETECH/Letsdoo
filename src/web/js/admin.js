...
import { apiFetch, getCsrfToken, requireAuth, showToast, syncSessionState } from './app.js';
...
function toggleAdminAccess(showAdmin) {
  if (showAdmin) {
...
async function loadDashboard() {
  try {
    ...
    if (error.message !== 'forbidden' && error.message !== 'Unauthenticated') {
...
async function initAdmin() {
  try {
    ...
    if (error.message && error.message !== 'forbidden' && error.message !== 'Unauthenticated') {
      showToast(error.message, 'error');
    }
  }
}
